...
const NotifyMeModal: React.FC<NotifyMeModalProps> = ({ isOpen, onClose, product, variant }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(err);
            addToast(err.response?.data?.message || 'Failed to set notification', 'error');
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="p-10">
                    ...
                    {!success ? (
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className="w-full bg-zinc-50 border border-zinc-100 rounded-xl px-4 py-4 text-sm md:text-xs font-bold outline-none focus:border-black transition-all text-black"
                                    value={email}
                        ...
                        <div className="py-12 text-center animate-in zoom-in duration-500">
                            ...
                            <p className="text-sm md:text-xs text-zinc-400 font-bold uppercase tracking-widest leading-loose text-center">
